...
describe('FlashLoan', () => {
  ...
  beforeEach(async () => {  
    ...
    let transaction = await token1.connect(deployer).transfer(liquidityProvider.address, tokens(1000000)) // NOTE: use 'connect' to connect to a contract
    await transaction.wait()

    transaction = await token2.connect(deployer).transfer(liquidityProvider.address, tokens(1000000)) // NOTE: "let" already done in first declaration
    await transaction.wait()
    ...
    transaction = await token1.connect(liquidityProvider).approve(amm1.address, amm1UsdAmount) // first person to add liquidity so sets the price
    await transaction.wait()
